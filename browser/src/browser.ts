...
export class TextMedia implements BaseMedia {
    ...
    async toBlob() {
        return new Blob([new TextEncoder().encode(this.content)], { type: this.mediaType })
    }
    async toBuffer() {
...
export function Media(mediaType: BinaryMediaType, contents: Buffer | string, encoding: Encoding): BinaryMedia
